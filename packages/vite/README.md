...
```ts
// 任意应用代码
import plan from "@skill/galgame/section-plan"; // 编译后 runtime 内容，@/ 引用自动内联
...
- **虚拟模块**：
  ...
  - `@skill/<path>` → 编译后 runtime 内容（frontmatter 已剥离），`@/path` 引用内联为自包含模块
- **配置**：`skillFramework({ skillsDir: "skills", outputDir: ".skill" })`。

## 定位

- **编辑器 md 书写层的 `@/` tip** → 用 [vscode-review-skill](../vscode-review-skill) 扩展（可选糖）。
